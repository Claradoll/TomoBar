...
namespace LittleTomato {
 ...
 public class Program {
  ...
  public StartupRegistration Startup;public Store Store; public Engine Engine;public MainWindow Main;public MiniWindow Mini;public TaskbarStrip Bar;public bool Quitting;public ImageSource IconSource;
  ...
  void Run(string[] args) {
   ...
   ApplyTheme();Main=new MainWindow(this);if(test)Main.Title="小番茄 · 测试 · "+StableHash(folder).Substring(0,6);application.MainWindow=Main;Mini=new MiniWindow(this);
   Main.SourceInitialized+=(s,e)=>{hwnd=new WindowInteropHelper(Main).Handle;Native.SetProp(hwnd,instanceTag,new IntPtr(1));HwndSource.FromHwnd(hwnd).AddHook(Hook);Native.RegisterHotKey(hwnd,1,0x4003,0x50);Native.RegisterHotKey(hwnd,2,0x4003,0x4F);};
  ...
  IntPtr Hook(IntPtr h,int msg,IntPtr wp,IntPtr lp,ref bool handled) {if(msg==0x312){if(wp.ToInt32()==1)ToggleTimer();else ShowMain();handled=true;}else if(msg==0x8001){ShowMain();handled=true;}else if(msg==0x8003){Exit();handled=true;}return IntPtr.Zero;}
  ...
  public void Exit(){if(Quitting)return;Engine.Pause();if(!Save()){ShowMain();return;}Quitting=true;if(timer!=null)timer.Stop();Native.UnregisterHotKey(hwnd,1);Native.UnregisterHotKey(hwnd,2);if(Bar!=null){Bar.Detach();Bar.Close();Bar.Dispose();}if(trayClickTimer!=null)trayClickTimer.Dispose();if(tray!=null){tray.Visible=false;tray.Dispose();}if(Mini!=null)Mini.Close();Native.RemoveProp(hwnd,instanceTag);Main.Close();application.Shutdown();}
  public void ApplyTheme(){bool dark=Data.Settings.Theme=="dark" || Data.Settings.Theme=="system"&&!Native.AppsLight;string[] keys={"Bg","Card","Ink","Muted","Line","Soft","Accent","Tint","Green","OnAccent"};string[] light={"#F6F5F2","#FFFFFF","#272B29","#69716A","#E7E8E2","#F0F1EC","#C45146","#FBEDE9","#63866A","#FFFFFF"};string[] night={"#1B1E1D","#242826","#F0F1EB","#A1A99F","#353C36","#303631","#E78778","#3E302C","#91B594","#1B1E1D"};for(int i=0;i<keys.Length;i++)application.Resources[keys[i]]=new SolidColorBrush((Color)ColorConverter.ConvertFromString(dark?night[i]:light[i]));if(Main!=null){int value=dark?1:0;Native.DwmSetWindowAttribute(new WindowInteropHelper(Main).Handle,20,ref value,4);Main.RefreshTimer();}}
  public void Backup(Window owner){Engine.Tick();if(!Save())return;var d=new SaveFileDialog {Title="备份小番茄数据",Filter="小番茄备份 (*.json)|*.json",FileName="小番茄备份-"+DateTime.Now.ToString("yyyyMMdd-HHmm")+".json"};if(d.ShowDialog(owner)==true){File.WriteAllText(d.FileName,Store.Encode(Data),new UTF8Encoding(false));UI.Notice(owner,"备份已保存。任务、设置与专注记录都包含在内。");}}
  public void Restore(Window owner){var d=new OpenFileDialog {Title="选择小番茄备份",Filter="小番茄备份 (*.json)|*.json"};if(d.ShowDialog(owner)!=true)return;AppData imported;try{var fi=new FileInfo(d.FileName);if(fi.Length>50*1024*1024)throw new IOException("备份文件超过 50 MB。");imported=Store.Normalize(Store.Decode<AppData>(File.ReadAllText(d.FileName,Encoding.UTF8)));}catch(Exception ex){UI.Notice(owner,"无法读取这个备份："+ex.Message);return;}if(MessageBox.Show(owner,"恢复后将替换当前任务、记录和设置。恢复前会自动保留一份当前数据备份。","恢复备份",MessageBoxButton.OKCancel,MessageBoxImage.Question)!=MessageBoxResult.OK)return;Engine.Pause();File.WriteAllText(Path.Combine(Store.DirectoryPath,"before-restore-"+DateTime.Now.ToString("yyyyMMdd-HHmmss")+".json"),Store.Encode(Data),Encoding.UTF8);Engine.Data=imported;ApplyTheme();Engine.Signal();Main.ShowPage("settings");UI.Notice(owner,"备份已恢复。未结束的计时以暂停状态恢复。");}
