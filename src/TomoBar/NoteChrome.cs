...
namespace LittleTomato {
 ...
 public partial class NoteWindow {
  ...
  void KeepOnScreen(){var handle=new WindowInteropHelper(this).Handle;Native.RECT rect;if(handle==IntPtr.Zero||!Native.GetWindowRect(handle,out rect))return;var work=NoteDocking.WorkArea(rect);int x=Math.Max(work.Left,Math.Min(rect.Left,work.Right-rect.Width)),y=Math.Max(work.Top,Math.Min(rect.Top,work.Bottom-rect.Height));Native.SetWindowPos(handle,IntPtr.Zero,x,y,0,0,0x1|0x4|0x10);}
