...
namespace LittleTomato {
 ...
 public static class UiSmoke {
  ...
  static void CheckTaskPreview(Program app,Action<bool,string> check,string folder){
   app.ShowMain();app.Main.ShowPage("tasks");var active=app.Data.Active;active.Elapsed=8;var runningTask=app.Engine.Current;var preview=new Todo{Title="仅预览另一项任务",Minutes=40};app.Data.Tasks.Add(preview);app.Engine.Signal();string selected=app.Data.Selected;int sessions=app.Data.Sessions.Count;
   app.Main.UpdateLayout();var card=Descendants(app.Main).OfType<System.Windows.Controls.Border>().First(b=>System.Windows.Automation.AutomationProperties.GetName(b)=="预览任务 "+preview.Title);card.RaiseEvent(new System.Windows.Input.MouseButtonEventArgs(System.Windows.Input.Mouse.PrimaryDevice,0,System.Windows.Input.MouseButton.Left){RoutedEvent=UIElement.MouseLeftButtonDownEvent});
   check(app.Main.PreviewedTask==preview&&Object.ReferenceEquals(active,app.Data.Active)&&active.Running&&app.Data.Selected==selected&&app.Data.Sessions.Count==sessions,"task single click only changes preview without switching or recording timer");
   check(Descendants(app.Main).OfType<System.Windows.Controls.TextBlock>().Any(t=>t.Text=="40:00"),"right panel displays preview task duration");
   check(Descendants(app.Main).OfType<System.Windows.Controls.Button>().Any(b=>String.Equals(b.Content,"结束并保存")&&!b.IsEnabled),"preview cannot end an unrelated active timer");Capture(app.Main,Path.Combine(folder,"13-task-preview.png"));
   var start=Descendants(app.Main).OfType<System.Windows.Controls.Button>().First(b=>System.Windows.Automation.AutomationProperties.GetName(b)=="切换并开始");start.RaiseEvent(new RoutedEventArgs(System.Windows.Controls.Button.ClickEvent));check(app.Data.Active.TaskId==preview.Id&&app.Data.Active.Running&&app.Data.Sessions.Count==sessions+1&&!app.Main.IsVisible,"explicit preview start switches task and saves previous focus");
   app.ShowMain();app.Engine.Pause();var paused=app.Data.Active;app.Main.PreviewTask(runningTask);check(Object.ReferenceEquals(paused,app.Data.Active)&&!paused.Running,"preview preserves paused timer");app.Main.PreviewTask(preview);paused.Elapsed=3;app.Main.RefreshTimer();check(Descendants(app.Main).OfType<System.Windows.Controls.TextBlock>().Any(t=>t.Text=="39:57"),"preview of active task displays actual remaining time");
   var resume=Descendants(app.Main).OfType<System.Windows.Controls.Button>().First(b=>System.Windows.Automation.AutomationProperties.GetName(b)=="继续专注");resume.RaiseEvent(new RoutedEventArgs(System.Windows.Controls.Button.ClickEvent));check(Object.ReferenceEquals(paused,app.Data.Active)&&paused.Running,"resume in preview does not restart active task");
   app.Engine.End();selected=app.Data.Selected;app.ShowMain();app.Main.PreviewTask(runningTask);check(app.Data.Active==null&&app.Data.Selected==selected,"idle preview does not commit selected task");
   var begin=Descendants(app.Main).OfType<System.Windows.Controls.Button>().First(b=>System.Windows.Automation.AutomationProperties.GetName(b)=="开始专注");begin.RaiseEvent(new RoutedEventArgs(System.Windows.Controls.Button.ClickEvent));check(app.Data.Active.TaskId==runningTask.Id&&app.Data.Active.Running,"start from idle preview starts the previewed task");
  }
  public static void Start(Program app,string folder){
   ...
   timer.Tick+=(s,e)=>{try{switch(stage++){
    ...
    case 9:
     check(app.Data.Active==null&&!app.Mini.IsVisible&&app.Main.IsVisible,"double click while stopped opens main without starting timer");
    ...
    case 11:
     ...
     CheckTaskPreview(app,check,folder);
     app.Main.Hide();app.Engine.Resume();app.Bar.ContextMenuStrip.Show(app.Bar,new System.Drawing.Point(0,0),Forms.ToolStripDropDownDirection.AboveRight);check(app.Bar.ContextMenuStrip.Visible,"taskbar context menu opens");Mouse(app.Bar,"OnMouseDown",1);Mouse(app.Bar,"OnMouseUp",1);check(!app.Bar.ContextMenuStrip.Visible,"left click closes taskbar context menu");break;
    case 12:
     check(app.Data.Active!=null&&app.Data.Active.Running,"menu-dismiss click does not pause timer after click interval");
     app.Bar.ContextMenuStrip.Show(app.Bar,new System.Drawing.Point(0,0),Forms.ToolStripDropDownDirection.AboveRight);SendMessage(app.Bar.Handle,0x1C,IntPtr.Zero,IntPtr.Zero);check(!app.Bar.ContextMenuStrip.Visible,"taskbar context menu closes on application deactivation");
