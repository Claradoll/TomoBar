...
下载 Release 中的 `TomoBar-1.5.1-win-x64.zip`，解压后运行 `小番茄.exe`。也可直接下载单文件 `TomoBar.exe`。无需安装、账号、联网或管理员权限。
...
在主界面「便签」页或任务栏右键菜单创建便签。直接输入，标题可留空；关闭窗口前会保存。便签采用整张纸色背景，格式操作集中在右键菜单，支持文字样式、段落与列表、链接、颜色、置顶和任务关联。勾选后文字变淡并加删除线，Ctrl+Z 可以撤销。
...
Windows 11 x64，使用系统自带的 .NET Framework 4.8 或更新版本。原生 WPF / WinForms，无浏览器内核或外部运行库；exe 约 215 KB。
