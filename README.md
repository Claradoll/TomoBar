...
下载 Release 中的 `TomoBar-1.5.2-win-x64.zip`，解压后运行 `小番茄.exe`。也可直接下载单文件 `TomoBar.exe`。无需安装、账号、联网或管理员权限。
...
在主界面「便签」页或任务栏右键菜单创建便签。直接输入，标题可留空；关闭窗口前会保存。便签采用整张纸色背景。右键菜单将文字样式、段落样式、列表与缩进分别归组；颜色、置顶与任务关联集中在「便签选项」。编号／分点列表可叠加「待办勾选」，也可用 Ctrl+1 切换勾选框。勾选后文字变淡并加删除线，Ctrl+Z 可以撤销。
