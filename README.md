...
下载 Release 中的 `TomoBar-1.6.0-win-x64.zip`，解压后运行 `小番茄.exe`。也可直接下载单文件 `TomoBar.exe`。无需安装、账号、联网或管理员权限。
...
便签右键「番茄专注 → 编号清单转为任务组」：标题成为组名，编号项成为独立番茄任务，缩进保留为分支。每项默认 1 个番茄、沿用默认专注时长，可单独调整。组标题可折叠，并汇总所有分支的投入与完成进度。

![编号清单生成的任务组](docs/images/task-groups-light.png)

便签与任务的勾选状态双向同步，任务名称随便签文字更新。右键某条清单可直接「专注此条任务」。新增或调整层级后使用「更新编号任务组」，已有时长和历史保留；删除正文条目不会删除任务与时间记录。
...
顶部图钉切换置顶，双击顶部可折叠／展开。折叠时只显示标题栏，拖动时自由移动，松手停在屏幕工作区边缘附近才会自动贴边；超出屏幕的部分会自动移回。底部贴边后折叠或展开，仍保持底边对齐。
...
Windows 11 x64，使用系统自带的 .NET Framework 4.8 或更新版本。原生 WPF / WinForms，无浏览器内核或外部运行库；exe 约 276 KB。
